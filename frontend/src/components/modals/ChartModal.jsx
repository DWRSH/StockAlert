  ...
  if (active && payload && payload.length) {
    return (
      <div className={`${isDarkMode ? 'bg-slate-800 border-slate-700 text-white' : 'bg-white border-slate-200 text-slate-900'} border p-3 rounded-lg shadow-xl text-xs font-mono`}>
        ...
        <p className="text-lg font-bold text-emerald-500">₹{payload[0].value}</p>
      </div>
    )
  }
  return null
                    ...
                    {isChartLoading ? (
                        ...
                        <ResponsiveContainer width="100%" height="100%">
                            <AreaChart data={chartData}>
                                ...
                                <YAxis domain={['auto', 'auto']} axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: theme.chartAxis }} tickFormatter={(val)=>`₹${val}`} />
                                <Tooltip content={<CustomTooltip isDarkMode={isDarkMode} />} cursor={{stroke: '#6366f1'}} />
